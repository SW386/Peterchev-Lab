...
rad=sqrt(pt(:,1).^2+pt(:,2).^2+pt(:,3).^2);
phi = atan2(pt(:,2),pt(:,1));
theta = acos(pt(:,3)./sqrt(pt(:,1).^2+pt(:,2).^2+pt(:,3).^2));
coeff=rand([2*globind(lmax,lmax) 1]);
Efield=(4*pi*10^(-7))*Efieldanat(pt,coeff,bigR,lmax)
E = gen_field(rad,bigR,theta,phi,lmax,coeff)

%coeff(globind(m,l))=
%coeff(globind(m,l)+globind(lmax,lmax))=

end
function Efield=Efieldanat(pt,coeff,rsphere,lmax)
globind=@(m,l) l+1+m*(lmax-(m-1)/2);
...
for l=0:lmax
    ...
    Br=radrr.*Ypos{2}(globind(0,l),:,1);Bth=radrr.*Ypos{2}(globind(0,l),:,2);Bph=radrr.*Ypos{2}(globind(0,l),:,3);
        ...
        Efield{globind(0,l)}(:,:,:,1)=0;
        Efield{globind(0,l)}(:,:,:,2)=reshape(...
            Bth(:),[nx(1) nx(2) nx(3) 1]);
        Efield{globind(0,l)}(:,:,:,3)=reshape(Bph(:),[nx(1) nx(2) nx(3) 1]);
        Efield{globind(0,l)+stack}=zeros([nx(1) nx(2) nx(3) 3]);
    
        Efield{globind(0,l)+stack}(:,:,:,1)=0;
        Efield{globind(0,l)+stack}(:,:,:,2)=reshape(Bth(:),[nx(1) nx(2) nx(3) 1]);
        Efield{globind(0,l)+stack}(:,:,:,3)=reshape(Bph(:),[nx(1) nx(2) nx(3) 1]);
    for m=1:l
    Br=radrr.*Ypos{2}(globind(m,l),:,1);Bth=radrr.*Ypos{2}(globind(m,l),:,2);Bph=radrr.*Ypos{2}(globind(m,l),:,3);
        ...
        Efield{globind(m,l)}(:,:,:,1)=0;
        Efield{globind(m,l)}(:,:,:,2)=reshape(Bth(:),[nx(1) nx(2) nx(3) 1]);
        Efield{globind(m,l)}(:,:,:,3)=reshape(Bph(:),[nx(1) nx(2) nx(3) 1]);        
    Br=radrr.*Yneg{2}(globind(m,l),:,1);Bth=radrr.*Yneg{2}(globind(m,l),:,2);Bph=radrr.*Yneg{2}(globind(m,l),:,3);
        ...
        Efield{globind(m,l)+stack}(:,:,:,1)=0;
        Efield{globind(m,l)+stack}(:,:,:,2)=reshape(Bth(:),[nx(1) nx(2) nx(3) 1]);
        Efield{globind(m,l)+stack}(:,:,:,3)=reshape(Bph(:),[nx(1) nx(2) nx(3) 1]);        
    end
end
end
